...
export const ToolsLite = (() => {
    ...
    _public.hasClass = (element, className) =>
    ...
    _public.setClass = (element, className, isActive) => {
        if (isActive) {
            _public.addClass(element, className);
        } else {
            _public.removeClass(element, className);
    ...
    var getRegEx = className => new RegExp(`(\\s|^)${className}(\\s|$)`);
     ...
     */
    _public.trim = str => str.replace(regEx.trim, "");

    _public.pack = value =>
        value.replace(regEx.dbleSpace, " ").replace(regEx.trim, "");
     ...
     */
    _public.getElementsByClass = (searchClass, node, tag) => {
        let i;
        let j;
