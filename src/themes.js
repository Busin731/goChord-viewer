import { css } from "./styles/themes.less";
 ...
 * and the Settings used to draw the diagrams.
 * 
 * @class Themes
 * @static
 * @singleton
...
export const Themes = ((() => {
    ...
    /**
     * available color schemes (see Settings)
    ...
    const setBody = themeName => {
        // JRM: cambia de body a div
        const container = document.getElementById(Settings.ids.container)
        ...
        for (const key in _colorSchemes) {
            if (_colorSchemes.hasOwnProperty(key)) {
                ToolsLite.removeClass(container, `theme-${key}`);
            }
        }
        ToolsLite.addClass(container, `theme-${themeName}`);
    };
     ...
     * @return {string}
     */
    _public.getDescription = themeName => _colorSchemes[themeName].selectText;
     ...
     */
    _public.loadList = (selector, selectedValue) => {
        let s = '';
        for (const key in _colorSchemes) {
            if (_colorSchemes.hasOwnProperty(key)) {
    ...
    /**
     * Sets body class and Settings to specified theme.
     * Change the color scheme requires reruning (to regenerate reference chord diagrams)
     * @method set
     * @param {string} themeName
     */
    _public.set = themeName => {
        setBody(themeName);

        const c = _colorSchemes[themeName];
        Settings.colors = c.song;
        Settings.tabs.lineColor = c.tabs.lines;
        Settings.tabs.dotColor = c.tabs.dots;
        Settings.tabs.textColor = c.tabs.text;
    };
