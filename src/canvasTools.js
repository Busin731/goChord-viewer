...
export const CanvasTools = (() => {
   ...
   * @return {void}
   */
  _public.drawDot = (ctx, centerPos, radius, color) => {
    ctx.beginPath();
   ...
   * @return {void}
   */
  _public.drawText = (ctx, pos, text, font, color, align) => {
    if (!ctx.fillText) {
      return; // IE check
    }
    ctx.font = font;
   ...
   */
  _public.addCanvas = (element, width, height) => {
    // make element
