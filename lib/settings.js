...
goChord.settings = (function() {
   ...
   * @type {enum_int}
   */
  _public.defaultInstrument = goChord.definitions.instrument.guitar;
   ...
   * @return mixed
   */
  var _scaleNode = function(node, multiplier) {
   ...
   * @return {void}
   */
  var _scaleFont = function(font, multiplier) {
    var bits = font.match(_sizeRe);
    if (bits.length < 2) {
      return font;
    }
    var size = parseInt(bits[1], 10) * multiplier;
