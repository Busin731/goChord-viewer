...
goChord.chordPainter = function () {
	...
	/**
	 * goChord.canvas object handle
	 * @property _brush
	 * @type goChord.chordBrush instance handle
	 * @private
	 */
	var _brush = null;
	 ...
	 * @method init
	 * @param htmlHandles {goChord.data.htmlHandles} DOM Element object
	 * @return {void}
	 */
	_public.init = function (htmlHandles) {
		_brush = new goChord.chordBrush();
		_brush.init();
	...
	var ignoreChord = function (chord) {
		for (var i = 0; i < goChord.settings.commonChords.length; i++) {
			if (chord == goChord.settings.commonChords[i]) {
				return true;
			}
		}
		return false;
	...
	_public.show = function (chords) {
		...
		if (goChord.settings.opts.sortAlphabetical) {
			chords.sort();
		}
		...
		for (var i = 0; i < chords.length; i++) {
			...
			if (goChord.settings.opts.ignoreCommonChords && ignoreChord(chords[i])) {
				if ((typeof Array.prototype.indexOf === 'function') && (_ignoreMatchList.indexOf(chords[i]) == -1)) {
					_ignoreMatchList.push(chords[i]);
				}
				continue;
			}
			var chord = goChord.definitions.get(chords[i]);
			if (!chord) {
				_errors.push(chords[i]);
				continue;
			}
			_brush.plot(_handles.diagrams, chord, goChord.settings.fretBox);
		}
		...
		for (var i = 0; i < chords.length; i++) {
			...
			for (var j = 0; j < e.length; j++) {
				if (e[j].getAttribute('data-chordName') == chord.name) {
					_brush.plot(e[j], chord, goChord.settings.inlineFretBox, goChord.settings.inlineFretBox.fonts);
				}
			}
		}
	};
