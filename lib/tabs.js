...
goChord.tabs = function() {
   ...
   * @type {JSON}
   */
  var tab_settings = goChord.settings.tabs;

  // TODO: use goChord.settings.tuning for NUM_STRINGS and LAST_STRING_NAME??
  ...
  /**
   * (Constant) Last String Name (Note). --JRM: la mas grave
   * @property LAST_STRING_NAME
   * @private
   * @type string
   */
  var LAST_STRING_NAME = "E";
  ...
  var loadBlocks = function(text, outElement) {
    ...
    var tab = [];
    for (var i in lines) {
      var s = goChord.toolsLite.trim(lines[i]);
      if (s.length > 0) {
        tab.push(s);
      }
  ...
  var redraw = function(inTabs, outElement) {
    ...
    var ctx = goChord.canvasTools.addCanvas(
      outElement,
   ...
   * @method readTabs
   * @private
   * @param gcStrings {array<string>} Block of tablbabure to be parsed
   * @return {2-dimentional array}
   */
  var readTabs = function(gcStrings) {
    var hasLabels = gcStrings[NUM_STRINGS - 1][0] == LAST_STRING_NAME;
    if (hasLabels) {
      stripStringLabels(gcStrings);
    }
    var frets = getFretNumbers(gcStrings);
    var symbols = getSymbols(gcStrings);
    var minLength = getMinLineLength(gcStrings);
   ...
   * @return {void}
   */
  var stripStringLabels = function(gcStrings) {
   ...
   * @return {void}
   */
  var getFretNumbers = function(gcStrings) {
    // first, get the frets
    var reInts = /([0-9]+)/g;
    var frets = [];
    for (var i = 0; i < NUM_STRINGS; i++) {
   ...
   * @return {void}
   */
  var getSymbols = function(gcStrings) {
    // convert to symbols
   ...
   * @return {void}
   */
  var getMinLineLength = function(gcStrings) {
    var minLength = 0;
    var line;
    var re = /-+$/gi;

    for (var i = 0; i < gcStrings.length; i++) {
  ...
  var getPackedLines = function(frets, symbols, guide, minLength) {
    ...
    var packed = [],
      ...
      stringIdx, // loop index for instrument's strings (guitar 6)
    ...
    for (var strIdx in tabs) {
      ...
      for (var chrIdx in tabs[strIdx]) {
        ...
        if (c == "|") {
          ...
          goChord.canvasTools.drawDot(
            ctx,
            center,
            settings.dotRadius,
            settings.dotColor
          );
          goChord.canvasTools.drawText(
            ctx,
            {
              x: center.x,
  ...
  var drawLabels = function(ctx, pos, settings) {
    // ['E','A','D','G','B','e'];
    var labels = goChord.settings.tuning.slice(0).reverse();
    for (var i = 0; i < NUM_STRINGS; i++) {
      goChord.canvasTools.drawText(
        ctx,
        {
