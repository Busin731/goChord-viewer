...
goChord.chordBrush = function() {
  ...
  _public.plot = function(
    ...
    if (!fontSettings) {
      fontSettings = goChord.settings.fonts;
    }
    if (!colorSettings) {
      colorSettings = goChord.settings.colors;
    }
  ...
  var _drawFretboard = function(ctx, pos, fretBox, fretColor) {
    ...
    // locals
    var stringHeight = goChord.settings.numFrets * fretBox.fretSpace;
    // JRM: cambia de 3 a 5
    var fretWidth = 5 * fretBox.stringSpace;
    // build shape
    ctx.beginPath();
    // add "C" & "E" strings
    ...
    // add frets
    for (i = 1; i < goChord.settings.numFrets; i++) {
   ...
   * @private
   * @param dots {array<data.dot>} Array of goChord.data.dot objects
   * @return {JSON}
   */
