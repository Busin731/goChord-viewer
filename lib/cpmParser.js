...
goChord.cpmParser = function() {
  ...
  _public.parse = function(text) {
    var song = new goChord.data.song();
    ...
    if (tmp.length > 0) {
      for (var i in tmp) {
        song.defs.push(goChord.chordImport.runLine("{define: " + tmp[i] + "}"));
      }
    }
    return song;
  };
    ...
    for (var i in song) {
      for (var j in song[i].lines) {
        if (regEx.instr.test(song[i].lines[j])) {
          ...
          tmpBlk.lines[0] = goChord.toolsLite.trim(args);
  ...
  var _markChordLines = function(song) {
    ...
    var chordFound, hasOnlyChords, line;
    ...
    for (var i in song) {
      ...
      for (var j in song[i].lines) {
        ...
        hasOnlyChords =
          chordFound &&
          goChord.toolsLite.trim(line.replace(regEx.allChords, "")).length < 1;
        // need to find
        song[i].lines[j] = {
          type: hasOnlyChords
