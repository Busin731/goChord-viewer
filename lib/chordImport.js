...
goChord.chordImport = (function() {
  ...
  var _lineToParts = function(line) {
    var s = goChord.toolsLite.pack(line);
    if (s.length > 1 && s[0] != "#") {
  ...
  var _getInstrument = function(text) {
    ...
    return !c ? null : goChord.toolsLite.pack(c[1]);
  };
  ...
  var _fretOMatic = function(text, frets, muted) {
    ...
    // JRM: cambia a 6
    var m = f[1].length == 6 ? f[1].match(regEx.any) : f[1].match(regEx.numOrX);
   ...
   * @param frets {array}
   * @param fingers {array}
   * @return {array<goChord.data.dot>} array of dots
   */
  var _toDots = function(frets, fingers) {
    var dots = [];
    var tuning = goChord.settings.tuning;
    for (var j = 0; j < tuning.length; j++) {
      var n = parseInt(frets[j], 10);
      if (n > 0) {
        dots.push(
          new goChord.data.dot(
            j,
            n,
   ...
   * @method _addInDots
   * @private
   * @param dots {array<goChord.data.dot>}
  ...
  var _getExpandedChord = function(text, adds) {
    ...
    var chrd = new goChord.data.expandedChord(name);
    // chrd.name = name;
   ...
   * @method runLine
   * @param line {string} Single line (string with one statment)
   * @return {goChord.data.expandedChord}
   */
   ...
   * @return {goChord.data.instrument}
   */
  _public.runBlock = function(text) {
    ...
    return new goChord.data.instrument(
