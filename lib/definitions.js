...
goChord.definitions = (function () {
	...
	_public.useInstrument = function (offset) {
		...
		if (_offset > 0) {
			_map = goChord.transpose.retune(_offset);
		}
		_public.setChords(goChord.chordImport.runBlock(_instruments[0]).chords);
	};
	...
	_public.get = function (chordName) {
		...
		for (i in _map) {
			if (name == _map[i].original) {
				...
				if (c) {
					chrd = new goChord.data.expandedChord(chordName);
					chrd.dots = c.dots;
					chrd.muted = c.muted;
					return chrd;
				}
